#version 330 core

in vec4 tColor;
in vec2 tUV;
in vec4 ShadowCoord;

out vec4 FragColor;

uniform sampler2D textureMap;
uniform sampler2DShadow shadowMap;

uniform bool showShadow;
uniform bool addBias;
uniform bool showAntiAliasing;
uniform bool showStratified;

float random(vec3 seed, int i){
	vec4 seed4 = vec4(seed,i);
	float dot_product = dot(seed4, vec4(12.9898,78.233,45.164,94.673));
	return fract(sin(dot_product) * 43758.5453);
}

void main(void)
{
    //FragColor = vec4(texture(textureMap, tUV).rgb , 1.0);

    	// Light emission properties
	vec3 LightColor = vec3(1,1,1);
	
	// Material properties
	vec3 MaterialDiffuseColor = texture( textureMap, tUV ).rgb;

vec2 poissonDisk[16] = vec2[]( 
   vec2( -0.94201624, -0.39906216 ), 
   vec2( 0.94558609, -0.76890725 ), 
   vec2( -0.094184101, -0.92938870 ), 
   vec2( 0.34495938, 0.29387760 ), 
   vec2( -0.91588581, 0.45771432 ), 
   vec2( -0.81544232, -0.87912464 ), 
   vec2( -0.38277543, 0.27676845 ), 
   vec2( 0.97484398, 0.75648379 ), 
   vec2( 0.44323325, -0.97511554 ), 
   vec2( 0.53742981, -0.47373420 ), 
   vec2( -0.26496911, -0.41893023 ), 
   vec2( 0.79197514, 0.19090188 ), 
   vec2( -0.24188840, 0.99706507 ), 
   vec2( -0.81409955, 0.91437590 ), 
   vec2( 0.19984126, 0.78641367 ), 
   vec2( 0.14383161, -0.14100790 ) 
);
	float bias = 0.000;
	if (addBias){
		bias = 0.005;
	}
	int index = 0; 
	float visibility = 1.0;
	if (showShadow){
		visibility -= 0.18 * 4 *(1.0-texture( shadowMap, vec3(ShadowCoord.xy,  (ShadowCoord.z-bias)/ShadowCoord.w) ));
		if (showAntiAliasing){
			visibility = 1.0;
			for (int i=0;i<4;i++){
				index = i;
				if (showStratified){
						index = int(16.0*random(FragColor.xyy, i))%16;
				}
				visibility -= 0.18*(1.0-texture( shadowMap, vec3(ShadowCoord.xy + poissonDisk[index]/1500.0,  (ShadowCoord.z-bias)/ShadowCoord.w) ));
		    }
		}

	}

	FragColor = vec4(visibility * MaterialDiffuseColor * LightColor, 1.0);
}